...
// "example" builtin's output file is itself editable input for `send
// --json`/`send --template`, and "randmsg --scenario <name>"/a compose
// randmsg job can use it as a subject/body seed, matched by a lowercased,
// space-stripped Title.
...
// ExampleTemplates are the 10 canned scenarios shared by the shell's
// "example"/"randmsg" builtins and compose's job kinds. Each exercises a
// handful of real template functions (SPEC.md §7.5.7) so a user gets a
// realistic, immediately-runnable starting point rather than a blank page,
// and can see functions in context before changing them.
